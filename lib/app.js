    ...
    App.prototype.run = function () {
        ...
        if (packageJson.scripts) {
            ...
            inquirer_1.default.prompt([
                ...
                var scriptProcess = child_process_1.exec("npm run " + script, function (err, stdo, stdi) {
                    if (err) {
                        console.error(err);
                    }
                });
                if (scriptProcess.stdout) {
                    scriptProcess.stdout.on('data', function (chunk) {
                        console.log(chunk);
                    });
                }
                if (scriptProcess.stderr) {
                    scriptProcess.stderr.on('data', function (chunk) {
                        console.error(chunk);
                    });
                }
            });
